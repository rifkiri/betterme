...
export const WeeklyOutputCard = ({
  ...
  const isOverdue = () => {
    return output.dueDate && isWeeklyOutputOverdue(output.dueDate, output.progress, output.completedDate);
  };
  return <>
      <div className={`p-4 rounded-lg border ${isOverdue() ? 'bg-red-50 border-red-200' : 'bg-blue-50 border-blue-200'}`}>
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            <div className="cursor-pointer hover:bg-blue-100 rounded p-1 -m-1 transition-colors" onClick={() => setEditingOutput(output)}>
              ...
              {output.isMoved && output.originalDueDate && <p className="text-xs text-orange-600 mb-1">
                  Moved from: {format(output.originalDueDate, 'MMM dd')}
                </p>}
            </div>
            {output.dueDate && <div className="flex items-center text-xs text-gray-500">
        ...
        <div className="flex items-center space-x-2">
          ...
          {output.progress !== 100 && <Button size="sm" variant="default" onClick={() => onUpdateProgress(output.id, 100)} className="text-xs px-2 py-1">Achieved</Button>}
        </div>
      </div>
      
      {editingOutput && <EditWeeklyOutputDialog weeklyOutput={editingOutput} open={true} onOpenChange={open => !open && setEditingOutput(null)} onSave={onEditWeeklyOutput} />}
    </>;
};